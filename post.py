...
def generate_post(topic: dict, recent_hooks: list[str] = None) -> tuple[str, str, str]:
    ...
    if case:
        case_block = f"""【実際の制作実績（この事例を題材にできる）】
業種：{case['industry']}
やったこと：{case['work']}
結果：{case.get('result') or '（結果は語らず、取り組み自体を題材にする）'}
背景・補足：{case.get('detail', '')}
※状況の肉付けや語り口は自然な範囲で工夫してよい（読者が情景を思い浮かべられるように）。
※ただし「PVが2倍」「売上◯◯％アップ」のような検証不能で大袈裟な数字は絶対に書かない。信用を損なう。
　結果は「増えた」「見つけてもらえるようになった」「楽になった」程度の等身大の表現に留める。
※テーマに合わなければ無理に事例を使わず一般論で書く。"""
    else:
        case_block = "【事例なし】大袈裟な嘘は作らず、一般的な役立つ話として書く。"

    # 投稿スタイルを重み付きで選択
    # 【2026-07-20 実測】実績シェア型は平均25views、それ以外は平均75views（3倍差）。
    # 読者は「鈴木さんの成功事例」ではなく「自分の問題」を読みたい。実績語りは最小限に下げた。
    styles = [
        {
            "name": "見落とし指摘型",
            "weight": 3.0,
            "main_hint": "読者が『気づいていない穴』『放置している状態』を、1行で具体的に指摘する。実測上位はすべてこの型（『Googleマップ、ホームページに繋いでますか。』337views／『5年前のサイト、まだ使ってます？』280views）。主語は常に読者の会社であって、鈴木さんの実績ではない。責める調子にはせず、そっと気づかせる",
            "body_hint": "その見落としが何を招いているかを説明し、確認方法と直し方を具体的に示す",
        },
        {
            "name": "ムダの可視化型",
            "weight": 2.5,
            "main_hint": "読者の日常に埋もれている時間のムダ・取りこぼしを、数字か情景で可視化する。実測上位（『議事録1件に45分。』242views／『見積書を打ちながら、商談の電話を逃した。』169views）。読者が『それ、まさに昨日の自分だ』と感じる具体的な場面を描く",
            "body_hint": "その作業がなぜ発生しているかを整理し、減らす手段を1つだけ具体的に提示する",
        },
        {
            "name": "実績シェア型",
            "weight": 0.4,
            "main_hint": "上記の【実際の制作実績】を題材にする。ただし主役は実績ではなく読者の学び。『〇〇業のサイトで△△したら□□になった』と淡々と述べ、すぐに『あなたの会社でも今日できること』へ話を移す。自慢や宣伝に絶対に聞こえないようにする",
            "body_hint": "何をどう変えたのかを見せ、「特別なことはしていない」という再現可能性を強調する",
        },
        {
            "name": "損失提示型",
            "weight": 1.0,
    ...
    cta_types = [
        ("配布型", "その投稿の悩みに直結する無料プレゼントを渡して登録動機を作る。例：『今日のチェックリストの完全版（全10項目）を、LINE登録した方にPDFで無料プレゼントしています。プロフィールのリンクから受け取ってください』。押し売りにならないよう、あくまで『役に立つものを置いておきますね』というトーンで", 0.45),
        ("宣言型", "「困ったら小田原の鈴木を思い出してください。プロフィールにLINEを置いてあります」のような、押しつけない存在表明", 0.20),
